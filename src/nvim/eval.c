...
static char *e_write_2 = N_("E80: Error while writing: %s");
    ...
    *arg = skipwhite(p + len);
    ...
    if (evaluate) {
      if (type_is && rettv->v_type != var2.v_type) {
        ...
        // it means TRUE.
        n1 = (type == TYPE_NEQUAL);
      } else if (rettv->v_type == VAR_BLOB || var2.v_type == VAR_BLOB) {
        if (type_is) {
          n1 = (rettv->v_type == var2.v_type
                && rettv->vval.v_blob == var2.vval.v_blob);
          if (type == TYPE_NEQUAL) {
            n1 = !n1;
          }
          ...
          n1 = tv_blob_equal(rettv->vval.v_blob, var2.vval.v_blob);
          if (type == TYPE_NEQUAL) {
            n1 = !n1;
          }
        }
  ...
  FUNC_ATTR_NONNULL_ARG(1)
{
  ...
  return true;
write_list_error:
  emsgf(_(e_write_2), os_strerror(error));
  return false;
}
...
{
  int error = 0;
  if (tv_blob_len(blob) > 0) {
    const ptrdiff_t written = file_write(fp, blob->bv_ga.ga_data,
                                         (size_t)blob->bv_ga.ga_len);
    if (written < (ptrdiff_t)blob->bv_ga.ga_len) {
      error = (int)written;
  ...
  return true;
write_blob_error:
  EMSG2(_(e_write_2), os_strerror(error));
  return false;
}
