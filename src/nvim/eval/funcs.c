...
static void f_getreginfo(typval_T *argvars, typval_T *rettv, FunPtr fptr)
{
  ...
  if (regname == '"') {
    ...
    dictitem_T *const item = tv_dict_item_alloc("isunnamed");
    item->di_tv.v_type = VAR_SPECIAL;
    item->di_tv.vval.v_number = regname == buf[0] ? VV_TRUE : VV_FALSE;
    tv_dict_add(dict, item);
  }
}
...
static void f_setreg(typval_T *argvars, typval_T *rettv, FunPtr fptr)
{
  ...
  if (regcontents && argvars[1].v_type == VAR_LIST) {
...
free_lstval:
    ...
    const char *strval = tv_get_string_chk(regcontents);
    if (strval == NULL) {
      return;
    }
