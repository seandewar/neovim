...
#define HAS_KEY_X(d, key) HAS_KEY(d, win_config, key)
  ...
  bool resize_only = (!has_vertical && !has_split)
                     || (was_split && !HAS_KEY_X(config, win) && old_split == fconfig->split);
  if (!resize_only && !check_split_disallowed_err(win, err)) {
    goto fail;  // error already set
  }

  // Merge config now as we may succeed past this point, or before autocommands/win_split_ins
  // possibly modify it!
